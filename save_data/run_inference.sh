...
main_folder="/n/scratch/users/f/fas994/wilson/fine-tune/train_800_img_rank_10_32_lr_1e4"
data_path="/n/scratch/users/f/fas994/wilson/data_csv/train_800_img_rank_10/train.csv"
...
while true; do
	...
	for subfolder in "$main_folder"/*/; do
		...
		if [[ ! " ${processed_folders[@]} " =~ " ${subfolder} " ]]; then
			...
			if [ -f "${subfolder}train.csv" ] || [ ! -f "${subfolder}adapter_model.safetensors" ]; then
				continue
			fi	
			...
			srun  	python tumor_infer.py \
				...
				--output_csv "${subfolder}train.csv" 
